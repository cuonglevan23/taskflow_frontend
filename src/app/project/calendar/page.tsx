...
export default function Component() {
  ...
  // Extended tasks with assignees and avatars
  const allTasks: Task[] = [
    // July 2025 tasks
    {
      id: 1,
        ...
        <div className="h-full overflow-y-auto">
          <div className="relative" style={{ minHeight: `${maxRows * 48 + 100}px` }}>
            ...
            {multiDayTasks.map((task) => (
              <div
                key={task.id}
                className={`${task.color} rounded-lg px-3 py-2 text-sm font-medium cursor-pointer hover:opacity-80 transition-opacity flex items-center gap-3 shadow-sm border border-white/50`}
                style={getMultiDayTaskStyle(task)}
                title={`${task.text} - ${task.assignee}`}
              >
                ...
                <div key={date.toISOString()} className="border-r border-gray-200 last:border-r-0 px-4 py-2 relative">
                  ...
                  <div className="relative" style={{ minHeight: `${maxRows * 48}px` }}>
                    {getSingleDayTasksForDate(date).map((task) => (
                      <div
                        key={task.id}
                        className={`${task.color} rounded-lg px-3 py-2 text-sm font-medium cursor-pointer hover:opacity-80 transition-opacity shadow-sm border border-white/50 flex items-center gap-3`}
                        style={{
                          position: "absolute",
                          top: `${(task.row || 0) * 48 + 8}px`,
                          left: "0",
